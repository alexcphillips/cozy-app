...
export class Registry {
    ...
    register(def: BaseContent) {
        ...
        if (store.has(def.id)) {
            throw new Error(
                `${APP_CONSTANTS.PREFIX_REGISTRY_ERROR} Duplicate id: ${def.id} in system ${def.system}`,
            );
        }
    ...
    get<T extends BaseContent>(system: System, id: string): T {
        ...
        if (!store) {
            throw new Error(
                `${APP_CONSTANTS.PREFIX_REGISTRY_ERROR} Missing system: ${system}`,
            );
        }
        ...
        if (!def) {
            throw new Error(
                `${APP_CONSTANTS.PREFIX_REGISTRY_ERROR} Missing definition: ${id} in ${system}`,
            );
        }
